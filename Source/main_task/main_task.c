...
static uint16_t segment_blink_timer = 0;
static uint8_t segment_display_on = 1;
...
static void key_SW(void)
{
    if (read_key & KEY_PUSH_FLAG)
    {
        ...
        switch (key_val)
        {
            case KEY_1: // Enter/Cycle through Time Setting Mode
                if (!key14_pressed) 
                {
                    key1_pressed = 1;
                    key1_count++;
            ...
            case KEY_8: // Decrement value (Down)
                if (key1_pressed) 
                {
            ...
            case KEY_14: // Enter/Cycle through Alarm Setting Mode
                if (!key1_pressed)
                {
                    key14_pressed = 1;
                    key14_count++;
